...
def frame_grab(counter):
    filename = CAPTURE_DIR + '/' + str(TRIAL) + '/image/' + str(counter).zfill(6) + '.jpg'
    counter += 1
    camera.start_preview()
    camera.capture(filename, 'jpeg', use_video_port = True)
    return counter

def frame_grabCV(counter):
    filename = CAPTURE_DIR + '/' + str(TRIAL) + '/image/' + str(counter).zfill(6) + '.png'
    counter += 1
...
def write_steer(counter, offset):
    filename = CAPTURE_DIR + '/' + str(TRIAL) + '/steer/' + str(counter).zfill(6) + '.txt'
...
def threadCapture(counter):
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        counter = frame_grabCV(counter, image)
